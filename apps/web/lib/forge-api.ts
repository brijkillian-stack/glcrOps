...
/** [start_HH:MM, end_HH:MM, duration_min] */
export type BreakSlot = [string, string, number];

export interface BreakSchedule {
  wave_labels: Record<string, string>;
  /** times[group_id][wave_id] = [start, end, duration_min] */
  times: Record<string, Record<string, BreakSlot>>;
}

export const DEFAULT_BREAK_SCHEDULE: BreakSchedule = {
...
export async function fetchBreakSchedule(): Promise<BreakSchedule> {
  try {
    return await get<BreakSchedule>("/v1/planning/settings/break_schedule", { cache: "no-store" });
  } catch {
    return DEFAULT_BREAK_SCHEDULE;
  }
}

export async function patchBreakSchedule(schedule: BreakSchedule): Promise<{ updated: boolean }> {
  const res = await fetch(`${BASE}/v1/planning/settings/break_schedule`, {
    method:  "PATCH",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify(schedule),
  });
  if (!res.ok) { const b = await res.text().catch(() => ""); throw new Error(`patchBreakSchedule ${res.status}: ${b}`); }
  return res.json();
}
