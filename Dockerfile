...
# ---------------------------------------------------------------------------
# API_URL / DEPLOY_URL are intentional hardcodes — both are baked into the
# compiled JS bundle at build time and must match the public Render service
# URL. Update both together if the Render service URL ever changes.
# (Dockerfile ENV doesn't support trailing inline comments — keep them above.)
